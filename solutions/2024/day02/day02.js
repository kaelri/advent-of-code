...
class Day02 extends Day {
    ...
    init() {
        ...
        let reports = input.map( ( line, l ) => {
            ...
            report.safe = this.isSafe(report);
        ...
        for (let r = 0; r < reports.length; r++) {
            ...
            if ( !report.safe ) {
                ...
                for (let v = 0; v < report.values.length; v++) {
                    ...
                    newReport.safe = this.isSafe(newReport);
    ...
    isSafe(report) {
        ...
        let expectedDirection = this.getDirection( diffs.map( diff => this.getDirection(diff) ).reduce( (total, direction) => total + direction, 0 ) );
        ...
        for (let d = 0; d < diffs.length; d++) {
            ...
            let direction = this.getDirection(diff);
